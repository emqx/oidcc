...
refreshes_after_timeout(_Config) ->
    ...
    %% third-party provider being reachable from CI. The response carries
    %% `Cache-Control: max-age=0' to force the worker onto the configured
    %% `fallback_expiry' (100 ms) — this also exercises the cache-control
    %% parser regression covered by emqx/emqx#17422.
